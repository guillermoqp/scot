...
class Valorizacion_model extends CI_Model {
    ...
    public function insert_valorizacion($numero, $anio, $mes, $fechaIn, $fechaFn, $idActividad, $idContrato, $idContratante) {
        ...
        $ordenes = $this->OTTomaEstado_model->get_all_orden_x_periodo_facturacion($idActividad, $fechaIn, $fechaFn, $idContratante);
        ...
        foreach ($subActividades as $subActividad) {
            ...
            foreach ($ordenes as $orden) {
                ...
                $lecturasControl = $this->Lectura_model->get_lecturas_control_capturador_x_orden_x_tipo($orden['OrtId'], $tipo);
                $lecturasAtip = $this->Lectura_model->get_lecturas_atipico_capturador_x_orden_x_tipo($orden['OrtId'], $tipo);

                $cantLecturas = count($lecturas) + count($lecturasControl) + count($lecturasAtip);
                $montoEjecOrden = $cantLecturas * $precioUnitario;
                $idOrdenSubActividad = $this->OTTomaEstado_model->insert_orden_subactividad($cantLecturas, $precioUnitario, $montoEjecOrden, $orden['OrtId'], $subActividad['SacId'], $idValOrdenSub);
    ...
    public function delete_valorizacion($idValorizacion) {
        ...
        foreach ($rows as $row) {
            ...
            $this->OTTomaEstado_model->delete_orden_subactividad_x_valorizacion($row['VloId']);
        }
        $this->db->trans_complete();
    }
