...
class OrdenValorizacion_model extends CI_Model {
    ...
    // OrdenValorizacion_ctrllr -> valorizacion_editar
    // OrdenValorizacion_ctrllr -> valorizacion_eliminar
    ...
    // OrdenValorizacion_ctrllr -> valorizacion_nuevo
    ...
    // OrdenValorizacion_ctrllr -> valorizacion_listar
    ...
    public function insert_valorizacion($numero, $anio, $mes, $fechaIn, $fechaFn, $idActividad, $idContrato, $idContratante) {
        ...
        $ordenes = $this->OrdenTrabajo_model->get_all_orden_x_periodo_facturacion($idActividad, $fechaIn, $fechaFn, $idContratante);
        // OBTENER LAS SUBACTIVIDADES DE LA ACTIVIDAD: TOMA ESTADO CON HOJA Y CON DISPOSITIVO
        ...
        foreach ($subActividades as $subActividad) {
            ...
            $acumMontoEj = 0;
            $tipo;
            if ($subActividad['SacCod'] == 'capturador') {
                //CON CAPTURADOR DE DATOS
                $tipo = TRUE;
            } elseif ($subActividad['SacCod'] == 'hoja_lectura') {
                //CON HOJA DE LECTURA
                $tipo = FALSE;
            }
            foreach ($ordenes as $orden) {
                // tipo = true = con capturador de datos
                $lecturas = $this->Lectura_model->get_lecturas_capturador_x_orden_x_tipo($orden['OrtId'], $tipo);
                $cantLecturas = count($lecturas);
                $montoEjecOrden = $cantLecturas * $precioUnitario;
                $idOrdenSubActividad = $this->OrdenTrabajo_model->insert_orden_subactividad(count($lecturas), $precioUnitario, $montoEjecOrden, $orden['OrtId'], $subActividad['SacId'], $idValOrdenSub);
                $acumMetradoEj += $cantLecturas;
                $acumMontoEj += $montoEjecOrden;
                $totalMetradoEj += $cantLecturas;
        ...
        foreach ($ordenes as $orden) {
            $totalMetradoPl += $this->Lectura_model->get_cant_lect_x_orden($orden['OrtId']);
            $totalDescuento += $this->OrdenTrabajo_model->get_descuento_x_idOrden($orden['OrtId']);
        }
        //VlrIgvVl => valor del igv
        $varIgv = $this->Variable_model->get_one_variable_cod('igv');
        $valorIgv = $varIgv['VarVal'];
        //VlrIgvMo => monto a restar
    ...
    public function delete_valorizacion($idValorizacion) {
        ...
        foreach ($rows as $row) {
            ...
            $this->OrdenTrabajo_model->delete_orden_subactividad_x_valorizacion($row['VloId']);
        }
        $this->db->trans_complete();
    }
    ...
    public function get_valorizacion_orden_subactividad_x_valorizacion($idValorizacion) {
        $query = $this->db->query('SELECT * FROM "Valorizacion_Orden_Subactividad" '
                . 'WHERE "VloEli" = FALSE AND "VloVlrId" = ? ORDER BY "VloId" DESC', array($idValorizacion));
        return $query->result_array();
    }
