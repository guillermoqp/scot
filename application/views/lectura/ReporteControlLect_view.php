    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <?php if (isset($mensaje)) { ?>
                <div class="alert alert-<?php echo ($mensaje[0] == 'error') ? 'danger' : 'success'; ?> alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    <?php echo $mensaje[1]; ?>
                </div>
            <?php } ?>
            <form id="mainForm" role="form" method="post" enctype="multipart/form-data">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">Control de Toma de Estado - Facturación , Periodo: <?php echo $mes ?> , Año: <?php echo $anio ?>  </h3>                                   
                    </div><!-- /.box-header -->
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="mes" name="mes" class="form-control" required="" onchange="cambiarMes(this)">
                                        <?php foreach ($meses as $nroOrden => $mesitem) { ?>
                                            <option value="<?php echo $nroOrden ?>"<?php echo ($mes == $mesitem) ? 'selected' : ''; ?>><?php echo $mesitem ?></option>
                                        <?php } ?>
                                    </select>
                                    <p class="help-block"></p>
                                </div>
                        ...
                        <table id="example1" class="table table-bordered table-striped" aria-describedby="example1_info">
                            <thead>
                                <tr Class="info" role="row">
                                    <th><?php echo isset($nivelSeleccionado) ? $nivelSeleccionado['NgrDes'] : 'Zona'; ?></th>
                                    <th>Fecha Lectura</th>
                                    <th>Lecturas Programadas</th>
...
function cambiarAnio(anio) {
    window.location = '<?php echo base_url() . 'toma_estado/reportes/control/' ?>' + $(anio).val() + '/<?php echo $mes[0] ?>';
};
function cambiarMes(mes) {
    window.location = '<?php echo base_url() . 'toma_estado/reportes/control/' . $anio . '/' ?>' + $(mes).val();
};
</script>
